...
select * FROM hundred_visits where id IN (select s1.id from hundred_visits s1 join hundred_visits s2 on (s1.id + 1 = s2.id) join hundred_visits s3 on (s3.id = s2.id + 1)
UNION ALL
